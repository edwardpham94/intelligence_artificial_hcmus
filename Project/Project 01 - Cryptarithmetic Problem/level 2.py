...
def checkcolumn(col,matrix,debt,alphab,operators,n,num):
    ...
    for permutation in itertools.permutations(available_digits, len(unknow_al)):
        ...
        if all(mapping[char] == digit for char, digit in zip(unknow_al, permutation) if digit):
            ...
            #số âm: phép toán sai nhưng có thể sửa được vì phép toán vế trái  bé hơn vế phải kết quả đủ số để 
            # lấp (VD 8=9? => 8+1=9) cái này xuất hiện nếu giả sử phép toán cột kế tiếp cộng dư 1 đơn vị
            #số dương: phép toán sai nhưng có thể sửa được vì phép toán vế trái  lớn hơn vế phải kết quả
            # đủ số để lấp (VD 9=8 => 9-1=8 => 9=8+1) cái này xuất hiện nếu giả sử phép toán cột
            # kế tiếp cần mượn cột mình để tính
            #None: HẾT CỨU
            #print(mapping)
            flag=True
            for c in con:
